...
### Requirement: EditorAssetProvider 模拟引用计数检测
EditorAssetProvider SHALL 维护模拟引用计数，LoadAsset 时 +1，ReleaseAsset 时 -1，场景退出 / OnDestroy 时检查是否有未配对的 Load/Release，若有则输出警告。

#### Scenario: Editor 模式检测未释放资源
- **WHEN** Editor 模式下 LoadAsset("Hero") 被调用 2 次，但只 ReleaseAsset("Hero") 1 次
- **THEN** 在 OnDestroy 或场景切换时输出警告日志："Hero" 存在未配对的 ReleaseAsset（loaded 2 times, released 1 time)

#### Scenario: Editor 模式正常配对的 Load/Release
- **WHEN** Editor 模式下 LoadAsset("Hero") 和 ReleaseAsset("Hero") 各调用 1 次
- **THEN** 系统不输出任何警告
